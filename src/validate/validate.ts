...
export const validateBody = (data: Record<string, any>, schema: Record<string,any>) => {
...
export const validateQuery = (data: Record<string, any>, schema: Record<string,any>) => {
    for (const key in schema) {
        const value = data[key];
        const field = schema[key];
        if (field.isRequired && (value === undefined || value === null)
        ) {
            throw new ErrorCustom(`Missing required query parameter: ${key}`, ErrorCode.BAD_REQUEST);
        }
        if(value === undefined) {
            continue;
        }
